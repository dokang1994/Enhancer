...
- The Gate 8 in-process worker (3a) and its documents are merged to `origin/main` through PR #7 (`6444aaa`); the AgentLoop-backed execution port and the `WorkPayload` execution-input extension are implemented locally on `main` and not yet committed.
