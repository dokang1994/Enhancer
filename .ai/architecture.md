...
- Delivery Gate 7 Event Bus and IPC Foundation is the sole `Specified - Next` product gate. Its `MessageEnvelope` contract is Contract Verified under `com.enhancer.bus`: versioned reference-only envelopes with canonical message/causation identities and a sealed four-kind payload hierarchy (work, result, control, handoff) carrying task revisions, snapshot identities, authorization scopes, run-record references, verification status, and control signals as data. Possessing an envelope grants nothing; delivery code must validate against repository authority. The next increment is deterministic in-process topic and queue delivery with idempotency and replay contracts.
...
- `GitWorkspaceCollector` holds the only external command authority, explicitly user-granted and decision-scoped: two fixed read-only git invocations, no shell, discovery confined to the project root, watchdog timeout, digest-only retention, and explicit UNAVAILABLE on every failure. The authority-boundary exit criterion is pinned by `WorkspaceAuthorityBoundaryIntegrationTest`. The next increment is bounded per-file Git metadata or a gate-level maturity assessment; diagnostics/terminal/selection adapters remain blocked on later-gate capabilities.
