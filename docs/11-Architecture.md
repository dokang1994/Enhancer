...
Current capability maturity is in `PROJECT_STATE.md` and the evidence behind it in `docs/verification-log.md`. Read them for the present position. This chapter previously restated maturity and went stale by a whole gate before anyone noticed, which is why the claim no longer lives here.
