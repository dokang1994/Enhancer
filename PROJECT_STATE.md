...
- Delivery Gate 8 process-isolated execution (connection 3d) under `com.enhancer.runtime`: `ProcessIsolatedAgentRunExecution` is the second production `AgentRunExecution`. It spools the work envelope to a `work/` spool under an invocation root private to the Goal and AgentRun, launches `IsolatedWorkerMain` through the `WorkerProcessLauncher` port with the project, evidence, and RunRecord roots as parent-supplied arguments, and returns the persisted RunRecord reference read back from a `result/` spool. The child runs the same Gate 1-4 pipeline as the in-process path through a shared `AgentLoopAgentRunExecution.executeWork` seam, so the two paths do not drift. The result is treated as a claim: the parent requires the envelope's correlation, logical-run, and causation identities to match the dispatched work, the payload to be exactly a `ResultPayload`, the task identity to match, the reference to resolve in the shared `RunRecordStore`, and the claimed verification status to equal the resolved record's own status, failing closed otherwise, so a child cannot promote its own run. A non-completed launcher outcome or any non-zero exit fails closed. Re-entry returns an already-published valid result without launching a second child; a child that persisted a RunRecord and died before publishing leaves an orphan and is re-executed under the existing at-least-once semantics. Store roots never come from payload data. Nothing wires it into `DurableAgentRunWorker`, spools are never cleaned up, and retry, cancellation, and concurrent cycles sharing an invocation root do not exist.
