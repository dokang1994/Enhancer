...
- Delivery Gate 8: Specified - Next; `WorkItem` admission, the dependency-ready single-worker queue, durable schema-v1 queue state/restart recovery, the durable one-Goal/one-AgentRun lifecycle, fenced single-owner lease/expiry recovery, durable queue terminal disposition, RunRecord-backed result-path finalization, the in-process Scheduler worker (3a), the AgentLoop-backed execution port, the `WorkPayload` execution-input extension, the isolated process lifecycle (3b), the local spool adapter (3c), and process-isolated execution (3d) are Contract Verified; the durable queue-to-lifecycle dispatch path and worker-over-real-execution path are Integrated. Connection 3d composes separate per-cycle work/result spools, runs the Gate 1-4 pipeline in a child, and accepts a result reference only after exact route/envelope/cardinality checks and RunRecord task/source/target/digest/status binding. It is not selected by `DurableAgentRunWorker` in a production composition, invocation roots have no retention policy, and the orphaned-RunRecord publication window remains an explicit at-least-once consequence. Effect records/fencing, retries, controls, and broader production wiring do not yet exist.
