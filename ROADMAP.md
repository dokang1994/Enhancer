...
- Integrated: target-file observation with real pre-run containment-checked digests, and read-only Git status/diff observation at digest granularity under explicitly granted, decision-scoped external command authority with discovery confined to the project root, inherited `GIT_*` overrides removed, and external diff/text-conversion helpers disabled;
