...
- next increment: return isolated worker results across the process boundary so connection 3's adapter and process lifecycle can be wired into `AgentRunExecution`; the child's only channel today is an exit code, which cannot carry a RunRecord reference;
