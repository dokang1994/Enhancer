...
- Gate 8: Specified - Next; immutable `WorkItem` admission, the dependency-ready single-worker queue, durable schema-v1 queue state/restart recovery, the durable one-Goal/one-AgentRun lifecycle, fenced single-owner lease/expiry recovery, durable queue terminal disposition, and RunRecord-backed result-path finalization are Contract Verified; durable queue-to-lifecycle dispatch is Integrated. `DurableAgentRunFinalizer` now connects a resolved RunRecord to the runtime terminal state and the matching queue disposition with autonomous post-terminal recovery, but the process-isolated worker that executes the Tool and produces the RunRecord, a concrete local IPC adapter, retry, effect records/fencing, and broader production wiring do not yet exist.
...
Integrate the Gate 8 process-isolated Scheduler worker and a selected local IPC adapter (connection 3): the worker executes the Tool, produces the RunRecord, and drives `DurableAgentRunFinalizer.finalizeAgentRun`, durably retaining the `runRecordReference` across the pre-terminal recovery window that the finalizer leaves to the caller. Transport acceptance never means bus delivery or work completion, and long-running Tool execution stays behind process isolation.
...
2. Confirm Gate 7 is `Contract Verified`, Gate 8 is the sole `Specified - Next` gate, and `CURRENT_TASK.md` records RunRecord-backed result-path finalization as Completed.
3. Inspect `git status --short` and the current branch log; the result-path finalization work lives on `feature/gate-8-result-path-finalization` and is not yet committed, pending explicit user authorization.
...
6. Activate the Gate 8 process-isolated worker and selected local IPC adapter (connection 3): execute the Tool, produce the RunRecord, and drive `DurableAgentRunFinalizer.finalizeAgentRun`, durably retaining the `runRecordReference` across the pre-terminal recovery window; preserve the distinction among execution acknowledgement, independently verified terminal state, and dependency satisfaction.
