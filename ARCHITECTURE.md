...
`GitWorkspaceCollector` holds the only external command authority in the codebase, explicitly granted and scoped by accepted decision: exactly two fixed read-only invocations (`git status --porcelain`, `git diff --no-ext-diff --no-textconv`) with no shell, inherited `GIT_*` overrides, external diff helpers, or text conversion. Both commands use `--no-optional-locks`, an invocation-scoped fsmonitor disable, discarded stderr, a watchdog-enforced five-second timeout, a four-MiB output cap, and repository discovery confined to the project root via a collector-owned `GIT_CEILING_DIRECTORIES`. Only the SHA-256 digest of each output is retained as `GIT_STATUS`/`GIT_DIFF` observation metadata; every failure, including a non-repository root or an absent git, is an explicit `UNAVAILABLE` observation. The authority-boundary exit criterion is pinned by `WorkspaceAuthorityBoundaryIntegrationTest`: adversarial tool-grant text in observed documents cannot widen task or policy scope, appear in bounded output, or mutate any document.
...
This contract is Contract Verified and consumed by the deterministic in-process topic and queue delivery surface below. Backpressure and IPC transport remain later increments over the same envelope.
...
This delivery, its failure handling, its bounded retry and explicit re-delivery, its cancellation propagation, and its run-to-completion ordering are Contract Verified. Backoff or delayed retry, priority ordering, competing queue consumers, backpressure over the unbounded pending queue, threading, persistence, and the IPC transport interface remain later increments over this surface.
