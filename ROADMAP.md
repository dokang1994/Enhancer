...
- Integrated sub-path: the real repository-derived work-message path now reaches a separate persist-first durable admission handler, which derives one stable distinct WorkItem identity, enqueues dependency-free work through the filesystem-backed queue before success, recovers the exact WorkItem from a fresh queue instance, and fails closed rather than duplicating work on fresh-bus re-delivery;
