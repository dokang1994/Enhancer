...
`GitWorkspaceCollector` holds the only external command authority in the codebase, explicitly granted and scoped by accepted decision: exactly two fixed read-only invocations (`git status --porcelain`, `git diff`) with no shell, `--no-optional-locks`, an invocation-scoped fsmonitor disable, discarded stderr, a watchdog-enforced five-second timeout, a four-MiB output cap, and repository discovery confined to the project root via `GIT_CEILING_DIRECTORIES`. Only the SHA-256 digest of each output is retained as `GIT_STATUS`/`GIT_DIFF` observation metadata; every failure, including a non-repository root or an absent git, is an explicit `UNAVAILABLE` observation. The authority-boundary exit criterion is pinned by `WorkspaceAuthorityBoundaryIntegrationTest`: adversarial tool-grant text in observed documents cannot widen task or policy scope, appear in bounded output, or mutate any document.
