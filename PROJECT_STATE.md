...
- Delivery Gate 8 replay-safe generated-input submission boundary under `com.enhancer.runtime`: `GeneratedInputSubmissionService` takes one caller-retained canonical submission UUID and derives the queue, correlation, and logical-run identities through fixed versioned domain-separated one-to-one UUID transforms (`GeneratedSubmissionIdentities`, `DERIVATION_VERSION = 1`), so the same key always names the same generated work across fresh stores. It resolves the existing `DurableSubmissionManifest` before consulting the clock or recapturing repository context: an absent manifest (`MissingSubmissionManifestException`) captures the occurrence time on first use through a `SubmissionEnvelopeFactory` and persists through the existing `DurableWorkSubmissionService`, while a present manifest reuses its exact occurrence time, envelope, and current default `NORMAL` priority and fails closed on any caller-owned intent conflict. No second durable store is added and no `scheduler-cycle`, execution, or polling is invoked; neither submission command yet accepts priority input.
- Delivery Gate 8 durable submission intent and queue-creation boundary under `com.enhancer.runtime`: `DurableSubmissionManifest` immutably binds one canonical message identity to the target queue identity, fixed capacity, required capability, exact `WorkPayload` envelope, and exact `NORMAL`/`EXPEDITED` Scheduler priority. `FileSystemSubmissionManifestStore` publishes one bounded schema-v2 strict-UTF-8 integrity artifact before queue creation, treats exact replay as a no-rewrite success, and rejects missing, corrupt, oversized, trailing, identity-mismatched, or changed-content/priority reuse. `DurableWorkSubmissionService` propagates that priority into dependency-free exact queue admission; the generic durable handler and existing submission construction paths retain `NORMAL` compatibility. Ordinary resolution rejects schema v1. Exact queue history remains the completion authority, while the filesystem queue store serializes each update through its queue-scoped local writer lock. No mutable receipt, execution, polling, automatic identities/time, public priority input, external adapter, or Gate 9 claim is added.
- Delivery Gate 8 stopped-submission manifest migration under `com.enhancer.runtime` and `com.enhancer.cli`: `FileSystemSubmissionManifestStore.migrateSchemaV1ToCurrent` plus `scheduler-migrate-submission-manifest` retain every schema-v1 field while assigning `NORMAL`. Typed absent/current outcomes do not rewrite an artifact; migration writes, rereads, and decodes a same-directory v2 candidate, rereads the unchanged source bytes, atomically replaces only after validation, and cleans the candidate on every path. Ordinary resolution remains v2-only. The command performs no queue creation, recovery, admission, claim, execution, Tool, effect, or priority input.
