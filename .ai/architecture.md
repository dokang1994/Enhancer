...
- Delivery Gate 7 Event Bus and IPC Foundation is the sole `Specified - Next` product gate. Its reference-only `MessageEnvelope` and deterministic single-threaded `InProcessMessageBus` are Contract Verified under `com.enhancer.bus`: topic fan-out, single-consumer queues, idempotency, journal replay, failure isolation, bounded retry and dead-letter re-delivery, correlation cancellation, and run-to-completion cascade ordering preserve the envelope and grant no authority. Replay-caused publications inherit non-journaling replay mode, and every publication reaches drain-owned admission. Backpressure over the unbounded pending queue is the next increment; persistence, threading, production wiring, and IPC remain deferred.
...
- The Gate 6 production graph composition is Operational: the CLI `run` path observes prior run records, the run's target file (containment-checked pre-run digest), and read-only Git status/diff digests into the snapshot, merges `AcceptedDecisionProjector` decision nodes and `TaskJustificationProjector` JUSTIFIED_BY edges resolved from the task document's optional `## Justified By` section, answers the task impact query, and reports bounded graph/impact counts. Malformed or unresolved justification references are rejected, never skipped.
- `GitWorkspaceCollector` holds the only external command authority, explicitly user-granted and decision-scoped: two fixed read-only git invocations, no shell, inherited `GIT_*` overrides, external diff, or text conversion, discovery confined to the project root, watchdog timeout, digest-only retention, and explicit UNAVAILABLE on every failure. The authority-boundary exit criterion is pinned by `WorkspaceAuthorityBoundaryIntegrationTest`; diagnostics/terminal/selection adapters remain owned by later gates.
