...
- accepted next continuation: add `DurableAgentRunRetryController` over the exact Goal ledger and persisted decision, without yet wiring replacement-attempt execution into the worker loop;
- deferred: general forward-reference graph/cycle handling, replacement AgentRun execution and retry-loop recovery, authenticated cancellation/pause/resume application, priority/fairness, budgets, authorized external-adapter execution and outcome evidence, checkpoints beyond current snapshots, schema-v1 runtime migration, power-loss directory durability, multi-process coordination, distributed clock-skew handling, and broader production wiring.
