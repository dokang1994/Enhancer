...
- Contract Verified: canonical queue identity and single-logical-run binding, immutable schema-v2 queue snapshots retaining every exact ordered admission including terminal work, bounded integrity-checked atomic filesystem persistence, persist-before-exposure admission/claim/disposition, revision-free exact admission replay, changed-content identity-reuse refusal, explicit schema-v1 rejection, and restart recovery that requeues interrupted active work in admission order under explicit at-least-once execution semantics;
...
- Accepted next boundary: replay-safe generated submission inputs use one caller-retained canonical submission UUID, versioned domain-separated derived identities, and the existing immutable submission manifest as the sole generated occurrence-time/envelope record. Replay resolves that manifest before clock or repository recapture, and submission remains separate from execution and polling;
- deferred: implementation and integration of that generated-input boundary, admission-history compaction/cleanup or schema-v1 queue migration, worker polling/service operation, general forward-reference graph/cycle handling, authenticated cancellation/pause/resume application, priority/fairness, broader budgets, authorized external-adapter execution and outcome evidence, checkpoints beyond current snapshots, schema-v1 runtime migration, power-loss directory durability, multi-process coordination, distributed clock-skew handling, and broader production wiring.
