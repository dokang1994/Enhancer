...
- Contract Verified: a pure, fail-closed `AgentRunRetryDecider` judges whether a further AgentRun may be admitted for one WorkItem over an immutable `AgentRunRetryPolicy` (1–16 attempts), a caller-supplied completed-attempt count, the terminal `WorkItemDisposition`, and the current external-effect ledger state. A non-`FAILED` disposition, any `PREPARED` effect, any `REQUIRES_USER_RECOVERY` effect, then an exhausted budget refuse in that fixed precedence, so an unresolved or recovery-pending effect blocks retry regardless of budget; it creates/persists/runs no AgentRun, mutates no store, and adds no wiring (connection #6 first slice, decision only);
- Accepted correction before connection #6 continues: the next implementation replaces terminal `WorkItemDisposition` input with the exact latest failed AgentRun, keeps the WorkItem active through `RETRY_PENDING`, admits automatic retry only for empty/all-`COMPENSATED` effect history, persists schema-v2 immutable attempt and retry-decision prefixes, splits result recording from terminal queue disposition, and proves every recovery prefix before any Integrated claim;
