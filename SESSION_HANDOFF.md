...
- The Gate 8 `WorkPayload` execution-input extension is now Contract Verified: `WorkPayload` (`com.enhancer.bus`) carries an optional caller-supplied `ExecutionInput(targetPath, expectedContentSha256)` (bounded non-blank target path to 1024 characters, 64-hex digest) with a three-argument convenience constructor that keeps every existing call site valid; `WorkItem.executionInput()` projects it; both filesystem serializers persist it after `allowedTools` behind a presence flag (schema v1 revised in place, pre-existing snapshots fail closed on the missing field, accepted for the unreleased artifact); and `WorkMessagePublisher` gained an overload carrying `Optional<WorkPayload.ExecutionInput>` as explicit caller authority data. `AgentLoopAgentRunExecution`'s single derivation seam now prefers the declared input and falls back to the approved source document, so a WorkItem executes an arbitrary governed target through the same contained read-file/evidence/verify/RunRecord pipeline while the `ApprovedTask` and Goal binding stay the source document.
- Producer decision pinned in the 2026-07-20 accepted decision: the execution input is caller-supplied (mirroring the CLI's `target-path`/`expected-sha256` authority model), never derived from Workspace snapshot observations (observations are evidence, not approval authority). No version bump or migration was added; multiple inputs, payload-carried plans/scripts, and write Tools stay out of scope. The named next connections are worker process isolation (3b) and the concrete local IPC adapter (3c).
- Fresh payload-extension evidence: aligned RED (exactly 30 test-compile errors naming only the absent `ExecutionInput`, four-argument constructor, and `executionInput()` accessor; production compiled), focused GREEN across the extended payload/projection/both-store-round-trip/publisher-overload/port-arbitrary-target/worker-end-to-end suites, the full 65-suite/299-test regression (297 passed, 2 existing Windows symbolic-link skips, 0 failures, 0 errors) under `--warning-mode all`, Java 17 strict lint across all 158 production sources, `git diff --check` clean, and exactly one `Specified - Next` marker (Gate 8). This work and the AgentLoop-backed port beneath it are local on `main` and not yet committed; the 3a worker is merged through PR #7 (`6444aaa`).
- The Gate 8 AgentLoop-backed execution port is Contract Verified and its worker path Integrated: `AgentLoopAgentRunExecution` (`com.enhancer.runtime`) is the first production `AgentRunExecution` — one `execute(dispatch)` call assembles the Integrated Gate 1-4 pipeline (governed `read-file` `ToolExecutor` with `EvidenceRecorder`-persisted evidence, bounded `AgentRunController`/`AgentLoop` at the CLI reference bounds of 5 iterations/stagnation 3/5-second timeout, `DeterministicReadFileVerifier`, application `AgentRunFinalizer`) and returns the persisted `run-record/<uuid>` reference. The governed target is the approved task's own source document (`taskRevision().sourceDocument()` as the `read-file` path, `taskRevision().sourceSha256()` as the expected content SHA-256), so no Gate 7 envelope, queue, or runtime serialization changed; a digest mismatch or Tool failure is carried in the persisted RunRecord (non-`VERIFIED`, finalized to the `FAILED` disposition), never thrown, and is real drift detection between approval and execution.
- Key decisions pinned in the 2026-07-20 accepted decision: the `ApprovedTask` is constructed directly from the WorkItem's fields (no `ApprovedTaskReader` — it is current-document-only and hard-fails unless `In Progress`, so it cannot reproduce an approved revision), `snapshotId` is not resolvable (no snapshot persistence), and a side execution-input store would split work authority away from the envelope; the port must persist through the same `RunRecordStore` the worker's `DurableAgentRunFinalizer` resolves from (mirroring the same-queue-instance rule); and the `(targetPath, expectedContentSha256)` derivation sits behind one private seam so the named follow-on `WorkPayload` execution-input extension (arbitrary targets, plus the publish-time producer design `WorkMessagePublisher` needs for the new fields) replaces only that derivation.
- Fresh port evidence: aligned RED (exactly 2 test-compile errors naming only the absent type), focused GREEN 3/3 (`AgentLoopAgentRunExecutionTest`: verified source document to a `VERIFIED` RunRecord with the taskId/sourceDocument/digest binding, digest mismatch to `REJECTED` non-completed with a resolvable reference, missing target to non-`VERIFIED`), and `FileSystemAgentLoopWorkerIntegrationTest` 2/2 on first run wiring the real port into `DurableAgentRunWorker` over shared real filesystem stores (verified claim plus dependent to `VERIFIED_COMPLETED` with two really persisted RunRecords and a cleared checkpoint; digest-mismatch claim to `FAILED` with the dependent blocked and the next cycle claiming nothing). Runtime package suite 16 suites/78 tests clean; full regression 65 suites/293 tests (291 passed, 2 existing Windows symbolic-link skips, 0 failures, 0 errors) under `--warning-mode all`; Java 17 strict lint across all 158 production sources; `git diff --check` clean; exactly one `Specified - Next` marker (Gate 8). This work is local on `main` and not yet committed; the 3a worker beneath it is merged through PR #7 (`6444aaa`).
- Gate 8 in-process Scheduler worker (connection sub-increment 3a) is Contract Verified: `DurableAgentRunWorker` (`com.enhancer.runtime`) drives one scheduling cycle end to end — claim + fenced lease through the existing dispatcher, execution through an injected `AgentRunExecution` port that persists a RunRecord and returns its reference, fence-checked `completeExecution`, then `finalizeAgentRun` recording the runtime terminal state and matching queue disposition — and recovers a cycle interrupted at any seam via a worker-owned durable single-record cycle-intent checkpoint (`PendingFinalization` in `FileSystemPendingFinalizationStore`).
