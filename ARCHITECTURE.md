...
Isolation is what makes termination possible. The in-process ceiling of 64 live workers contains stuck code but cannot stop it, and that ceiling still governs in-process execution. `ProcessIsolatedAgentRunExecution` wires the launcher and file-spool adapter into the execution port; no production composition selects it for `DurableAgentRunWorker`.
...
Which connections exist today is stated in `PROJECT_STATE.md`; remaining production connections follow this dependency order:
...
| 3 | process-isolated worker and local IPC (3a-3d contracts exist; worker selection and spool retention remain) | Gate 7 transport + Gate 8 worker runtime + Gate 11 Tool controls | worker cycle-intent persists before the claim and the RunRecord reference persists before acknowledgement; exact work/result route and record binding fail closed; transport acceptance never means bus delivery or work completion |
...
Each cross-store step persists its earlier authoritative artifact before the later derived artifact. Recovery re-enters idempotently from the durable prefix; it does not claim an atomic transaction. Worker selection of the process-isolated port, spool retention, durable control consumption, effect ledger, retry policy, and handoff coordination remain unimplemented until their own bounded tasks and fresh integration evidence exist.
