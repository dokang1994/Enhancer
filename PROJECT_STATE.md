...
- Production source: 167 Java files.
- Test source: 71 Java files.
...
- Delivery Gate 7: Contract Verified after a fresh Integrated maturity assessment. The work-message queue/journal/replay/idempotency path is Integrated, and connection 3d gives `MessageTransport` one named local work/result-spool consumer. Result/control/handoff and non-empty-causation flows, topic and failure/retry/dead-letter/cancellation/cascade-ordering/backpressure branches remain contract-only; no durable bus or supported messaging entry point exists.
- Delivery Gate 8: Specified - Next; `WorkItem` admission, the dependency-ready single-worker queue, durable schema-v1 queue state/restart recovery, the durable one-Goal/one-AgentRun lifecycle, fenced single-owner lease/expiry recovery, durable queue terminal disposition, RunRecord-backed result-path finalization, the durable Scheduler worker (3a), the AgentLoop-backed execution port, the `WorkPayload` execution-input extension, the isolated process lifecycle (3b), the local spool adapter (3c), and process-isolated execution (3d) are Contract Verified; the durable queue-to-lifecycle dispatch path, worker-over-real-execution path, and process-isolated durable worker composition are Integrated. The process-isolated composition uses separate per-cycle work/result spools, the real child launcher, and exact route/envelope/cardinality plus RunRecord task/source/target/digest/status binding. Successful spools retire only after reference checkpointing; failed or incomplete current cycles retain their spool, and the orphaned-RunRecord publication window remains an explicit at-least-once consequence. Effect records/fencing, retries, controls, a supported entry point, and broader production wiring do not yet exist.
