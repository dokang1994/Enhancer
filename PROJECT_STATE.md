...
- Delivery Gate 8 durable queue state and restart recovery under `com.enhancer.runtime`: canonical queue identity and one-logical-run binding, immutable schema-v2 snapshots retaining every exact `QueuedWork` in admission order including terminal work, a bounded strict-UTF-8 integrity-checked atomic filesystem store that rejects history rewrite/truncation across revisions, monotonic revisions, persist-before-exposure transitions, and admission-ordered active-work requeue on restart. Each update opens a stable queue-scoped lock artifact without following links, takes a non-blocking operating-system file lock across current-state resolution, revision/history validation, and atomic publication, and reports `ConcurrentSchedulerQueueUpdateException` without mutation when another local JVM or overlapping instance owns it. A real child-JVM integration proves contention refusal and unchanged queue state, while a stale-store test proves a committed revision cannot be overwritten. Strict `enqueue` still rejects duplicates; the production admission handler uses a separate exact idempotent boundary where replay is revision-free and changed capability, envelope, authorization, provenance, execution input, or dependencies under the same identity fail closed. The lock artifact carries no queue or authority state. Schema-v1 artifacts are explicitly unsupported; no migration, history compaction, lease, fence, effect deduplication, distributed lock, broader cross-store coordination, network-filesystem, or power-loss directory-sync claim exists.
...
- The supported Gate 8 migration exit-criterion slice is satisfied for the only schema
  currently declared migratable. The named migration-to-cycle fixture proves that a
  ...
  clears the checkpoint. Queue, runtime, and external-effect schema v1 remain
  intentionally unsupported because their missing current-contract information cannot
  be reconstructed; this bounded satisfaction does not promote Gate 8 as a whole.
