...
- The Gate 6 production graph composition is Operational: the CLI `run` path observes prior run records into the snapshot (`RunRecordMetadataCollector` over the store's read-only `references()` listing, corruption surfaced as UNAVAILABLE), merges `AcceptedDecisionProjector` decision nodes and `TaskJustificationProjector` JUSTIFIED_BY edges resolved from the task document's optional `## Justified By` section, answers the task impact query, and reports bounded graphNodes/graphEdges/graphDecisions/impactExecutions/impactDecisions counts. Malformed or unresolved justification references are rejected, never skipped. The next increment is the next read-only source adapter or gate-exit evidence; a Git adapter needs an explicit external-command authority decision.
