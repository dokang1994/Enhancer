...
- The Gate 6 task impact query is Contract Verified: `TaskImpactQuery` answers the task-to-decision-to-code-to-test chain over one projected graph with snapshot-traceable immutable results and rebuild status derived from every traversed element; transitive DEPENDS_ON closure is deferred by decision.
...
- The Gate 6 production graph composition is Operational: the CLI `run` path observes prior run records into the snapshot (`RunRecordMetadataCollector` over the store's read-only `references()` listing, corruption surfaced as UNAVAILABLE), merges `AcceptedDecisionProjector` decision nodes parsed from the decision log's own status lines, answers the task impact query, and reports bounded graphNodes/graphEdges/graphDecisions/impactExecutions counts. Decisions are unlinked until a task-to-decision reference grammar is adopted. The next increment is that reference grammar with JUSTIFIED_BY projection, or the next read-only source adapter; a Git adapter needs an explicit external-command authority decision.
